    ...
    <section className="">
      ...
      <div className='mt-8 mx-auto justify-items-center grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-white'>
        {cortado.map((proyecto) => (
          <motion.div className='cursor-pointer' onClick={() => btnOpenModal(proyecto)}
            viewport={{ once: true }}
      ...
      <AnimatePresence mode='wait'>
        ...
        {selectedProyect && openModal && (
          <motion.div
            className={`fixed inset-0 bg-black/70 flex items-center justify-center z-50 modal ${openModal ? 'show' : ''}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
